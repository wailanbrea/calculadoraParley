import React from 'react';
import NotificacionesControles from './NotificacionesControles';

export default function Navigation({ activePage, setActivePage }) {
  return (
    <aside className="sidebar">
      <div className="brand-section">
        <span className="brand-logo">⚾</span>
        <h1 className="brand-name">BSolutionsParley</h1>
      </div>
      
      <nav>
        <ul className="nav-links">
          <li className="nav-item">
            <button 
              className={`nav-link ${activePage === 'dashboard' ? 'active' : ''}`}
              onClick={() => setActivePage('dashboard')}
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6ZM13.5 15.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z" />
              </svg>
              Panel Principal
            </button>
          </li>
          <li className="nav-item">
            <button 
              className={`nav-link ${activePage === 'parley' ? 'active' : ''}`}
              onClick={() => setActivePage('parley')}
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 15.75V18m-3-3V18m-3-3v3m3-9h.008v.008H12.75V6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V8.25A2.25 2.25 0 0 0 18.75 6h-7.5Z" />
              </svg>
              Calculadora Parley
            </button>
          </li>
          <li className="nav-item">
            <button 
              className={`nav-link ${activePage === 'tickets' ? 'active' : ''}`}
              onClick={() => setActivePage('tickets')}
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.5 6v.75m0 3v.75m0 3v.75m0 3V18m-9-12v.75m0 3v.75m0 3v.75m0 3V18M3 8.25h18M3 12h18m-18 3.75h18M3 15.75h18" />
              </svg>
              Fraccionar Tickets
            </button>
          </li>
          <li className="nav-item">
            <button 
              className={`nav-link ${activePage === 'bases_alcanzadas' ? 'active' : ''}`}
              onClick={() => setActivePage('bases_alcanzadas')}
            >
              <span style={{ fontSize: '1.2rem', width: '24px', textAlign: 'center' }}>⚾</span>
              MLB
            </button>
          </li>
          <li className="nav-item">
            <button
              className={`nav-link ${activePage === 'basket' ? 'active' : ''}`}
              onClick={() => setActivePage('basket')}
            >
              <span style={{ fontSize: '1.2rem', width: '24px', textAlign: 'center' }}>🏀</span>
              Básquetbol
            </button>
          </li>
          <li className="nav-item">
            <button
              className={`nav-link ${activePage === 'soccer' ? 'active' : ''}`}
              onClick={() => setActivePage('soccer')}
            >
              <span style={{ fontSize: '1.2rem', width: '24px', textAlign: 'center' }}>⚽</span>
              Soccer
            </button>
          </li>
          <li className="nav-item">
            <button
              className={`nav-link ${activePage === 'comparator' ? 'active' : ''}`}
              onClick={() => setActivePage('comparator')}
            >
              <span style={{ fontSize: '1.2rem', width: '24px', textAlign: 'center' }}>🔍</span>
              Comparador
            </button>
          </li>
          <li className="nav-item">
            <button
              className={`nav-link ${activePage === 'settings' ? 'active' : ''}`}
              onClick={() => setActivePage('settings')}
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.43l-1.003.828c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.43l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28Z" />
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
              </svg>
              Tabla
            </button>
          </li>
        </ul>
      </nav>

      <NotificacionesControles />
    </aside>
  );
}
